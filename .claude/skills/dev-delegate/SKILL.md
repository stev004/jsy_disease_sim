...
## Operator visibility — make every worker identifiable

Every Codex brief starts with these two lines, before any other content:
```
TASK: <project> · <short slug> · <plain-English action, 72 chars max>
ROLE: implementer | reviewer | consultant
```
Use a specific action (`brandreel · script-stage · build validated script JSON`), never a generic label (`implementation`, `agent 1`, `fix stuff`). Retries keep the same TASK and append `· retry N`; parallel units must have distinct slugs. CodexBar derives its descriptive session label from the opening brief, so putting TASK first makes the menu useful; the worktree/project label remains the second identity check.

Immediately before launching, tell Steven one compact line: `Delegating: <TASK> | <ROLE> | <model@effort> | <worktree>`. When it finishes, report the same TASK followed by `done`, `retrying`, `blocked`, or `failed`. Never make Steven infer which PID, worktree, or generic "implementer" belongs to which job.
...
3. **Worktree — isolate every job, branch from fresh default-branch.** Preferred since 09-02: the deterministic runner `~/Documents/regulate-hivemind/scripts/job.sh` (generic — works on any repo, not just Regulate): `job.sh setup <repo> <type>/<slug>` → `job.sh delegate <worktree> <spec-file>` (wraps the codex invocation + 20-min zero-write watchdog; exit 3 = wedge, no budget cost) → `job.sh gates <worktree>` (auto-detects typecheck/lint/test/build) → `job.sh ship <worktree> "<msg>"` (refuses main/master, strips `.env`) → `job.sh cleanup <worktree>`. Manual fallback:
   ```
...
4. **Delegate to Codex.** Routing (update this block when the fleet changes — routing here is intentionally independent of Regulate's MODEL_ROUTING.md; review this block whenever the fleet routing changes; Regulate implementation default = luna@xhigh since 09-02 (MODEL_ROUTING v6); orchestrator/consultant seat = Fable 5.1 by default since 09-01 — if you are not Fable, taste/design/architecture calls go to a `model: "fable"` subagent): **gpt-6-luna @ xhigh** is the implementation workhorse (the only luna effort in the loop — 09-02; `medium` allowed for trivial mechanical renames/boilerplate only). **gpt-6-sol @ high** is the peer engineering consultant and escalation tier. **gpt-6-astra @ high** = second opinion / fallback / trail audit (v7.3, 2026-09-25: current-generation frontier model, independent of the implementer and reviewer; replaces gpt-5.5, which was two generations behind; Terra discontinued 2026-09-22). GPT-6 fleet since 2026-09-22 (Steven; MODEL_ROUTING v7): Luna/Sol moved 5.6 → 6; **gpt-6-astra** = the Codex director seat (`$foreman`). Requires codex-cli ≥ 0.155 — older CLIs reject GPT-6 slugs ("requires a newer version" / "not supported with a ChatGPT account").
   > **Known-good invocation:** `cd <worktree> && codex exec --sandbox workspace-write -m gpt-6-luna -c model_reasoning_effort="xhigh" "<spec>" < /dev/null`
...
- Prompt-cache exception: the short `TASK:` line is intentionally task-specific so CodexBar is readable. Keep `ROLE:` and the remaining brief structure stable; never put timestamps or opaque run IDs in TASK.
