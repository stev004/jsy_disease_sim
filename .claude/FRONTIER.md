...
**Updated:** 2026-09-04 late (validation PASS; main CI red root-caused + fixed on `fix/checkpoint-root-outside-worktree` @ `d873a80`, awaiting G14) · **Updated by:** Fable

## Where the project is

**Tier: V1.1 RELEASED; V1.2 cycle open.** `main` = **`43008ff`** (2026-09-04) = tag `jos-v1.1.0` (`e502ebfd…`) + state layer + carry-ins (G7) + R6 (G10) + frozen Jersey COVID snapshots (G11) + R7 perf chain (G12) + R8 audit chain (G13); suite 284. The release itself was fast-forwarded, smoked, and pushed 2026-09-01 (merge executed by the agent on Steven's explicit chat instruction — trail row of same date). V1.0 remains tagged `jos-v1.0.0` at `9e9ce3ab...`.
...
1. **P4 — DONE 2026-09-03** (44/44; validation rerun on R8 code in flight 2026-09-04). G8 ruling: ≥40 successful replicates for 2.5/97.5 bands (n·min(q,1−q)≥1 rule). Bands are stochastic replicate variation, never confidence intervals. *(The 2026-09-01 machine note claiming 1.8 GB/replicate and ~12 workers was wrong — see the measured memory model above; the Mac release run's memory pressure is explained by the same pre-R6 cache growth.)*
...
**Live:** `main` @ `43008ff` (= `jos-v1.1.0` + carry-ins + G10 R6 + G11 snapshots + G12 R7 + G13 R8) · `docs/jos-v1-scientific-review` @ `b8aeb8b`.
**Merged, preserve:** R6 pair (`79ef7b2`, `3617a91`) · `feat/v12-epi-snapshots` (`f5fba15`) · R7 chain tip (`3213314`) · R8 chain tip (`2528733`) and its 8 unit branches (`codex/r8-*`).
