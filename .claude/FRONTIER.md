...
**P4 — IN FLIGHT (relaunched 2026-09-02 11:24Z on DESKTOP-KQTC6VL/WSL2, `--workers 8`).** G8 resolved (Steven: default). First launch (04:56Z, `--workers 12`) was killed at ~11:20Z: kernel OOM-killed a pool worker at 06:27 (12 × ~2.3 GB > 26 GB WSL cap; dmesg evidence), `BrokenProcessPool` is a `RuntimeError` so `run_ensemble` silently fell back to sequential (py-spy stack at `ensemble.py:665`) — honest ETA had become 4–5 days. Relaunch: 44 seeds (101–144), `--mode full`, 180 days, `--workers 8` (fits: 8 × ~2.5 GB + parent ≈ 26 GB cap), `PYTHONUNBUFFERED=1`, output `~/Documents/JOS_v1_2_full_scale_evidence/`, log `p4-ensemble-launch-20260902T112435Z.log`, pid file `p4-ensemble.pid`. Est. ~15–18 h wall (44/8 = 6 waves). Next action once it completes: verify ≥40 successes + `actual_workers`/`execution_mode` in diagnostics, summarize bands (stochastic replicate quantiles, never CIs), file the run report. Original context below. The V1.2 carry-ins are on `main` (merge `9a2d984`, from `codex/v1.2-carry-ins` @ `9711b8e3937b3ff18aec86523ed4769ff78cfd4c`; CI run 33556105665 green; suite 235): travel-ensemble diagnostics status derived from named predicates (Sol Pro §12.8) · full release gate in CI — `uv lock --check`, compileall, real-CLI M7 generate→copy→delete-original→verify (`scripts/ci_relocation_check.py`, refuses pre-existing artifacts), `git diff --check`, clean-tree assertion, frontend job (M03 / §12.3) · `jos verify bundle-selftest` writing a machine-readable relocation transcript at bundle level (§12.6; exhibit against the real release artifact at `docs/runs/2026-09-01-v12-release-artifact-bundle-selftest.json`). DIRECTOR.md hard rules are at the released state. Trail audit (terra): 6 flags, all closed (`docs/runs/2026-09-01-v12-carry-ins-trail-audit-terra.md`).
