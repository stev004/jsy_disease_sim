...
1. Read `docs/roadmap.md` NEXT → this file → `.claude/GATES.md` (open: **G16**, **G17**, G5) → `tail -5 .claude/decisions.tsv` (last rows: `v12-run2-close`, `v12-run2-trail-audit`).
2. If Steven has merged G16/G17: `git -C ~/jsy_disease_sim pull --ff-only` in WSL; read `gh run view --json jobs` for the merge commit's CI and log it (`fm.sh log`). If not merged: work continues on a NEW branch off `feat/v12-denominators-dictionary` @ `4465453` (it contains everything) — do not branch the corrective unit off `main`, the dictionary lives on that branch.
