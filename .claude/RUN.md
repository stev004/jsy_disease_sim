...
**In flight — iteration 4 (corrective unit on the same branch):** Codex luna@xhigh, worktree `~/jos-v12-iter2-wt`, brief `~/jos-v12-iter4-brief.md`, log stem `~/jos-v12-iter4`. Then iteration 5 = re-audit (Sol, fresh clone at the new SHA).
- **Cold-start recipe:** as before; after the corrective commit is pushed and CI is green, clone fresh at the new SHA and relaunch the audit brief (`~/jos-v12-exit-audit-brief.md` with `__CLONE_DIR__`/`__AUDIT_SHA__` substituted) with `fm.sh exec <clone> gpt-5.6-sol high <brief> ~/jos-v12-exit-audit-2`.
