...
- **Default:** MERGE. Three exit-gate audits (all filed in `docs/audits/`) confirm the branch's tables rebuild byte-identically and trace to frozen cells; the remaining FAIL findings are measure-dictionary/contract items that the next run corrects on a new branch from `main`. Holding this branch unmerged only delays the evidence foundation.
