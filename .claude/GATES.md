...
- **Option B (extended candidate):** `perf/integration-tranche2b` @ `0dec469f83ee936b9b68e979e7a927807d7654e0` = the same plus ROUTE-5 phase 1 (columnar snapshots; exact on all fingerprints; ~8× smaller snapshot memory) + ROUTE-4 (workplace_team weekday memo, 5.7×) + the director's merge resolution. Becomes the recommended candidate only if Sol review 2 (running) returns PASS and its CI is green — RUN.md records the verdict. Command then: `git -C ~/jsy_disease_sim merge --no-ff 0dec469f83ee936b9b68e979e7a927807d7654e0 -m "G18: merge perf/integration-tranche2b @ 0dec469 (PERF-1/2/3 + ROUTE-5 phase 1 + ROUTE-4)" && git -C ~/jsy_disease_sim push`.
- **Default:** merge option A (`ac04bbb`, reviewed + CI green) if review 2 is not PASS; merge option B if it is. Agent does not merge without an explicit instruction in chat.
