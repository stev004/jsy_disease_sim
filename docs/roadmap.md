...
- ☐ Age-band denominators derived from the annual estimates (vaccination bands + 16+ + 50+), with an explicit band→age mapping (iteration 2).
- ☐ **Measure dictionary** `data/processed/measure_dictionary.csv` — one row per (table, measure): event-date definition, geography/universe, units/denominator, suppression semantics, reporting regime, known exclusions; `unknown` allowed, inference banned (required by the exit gate, see `docs/research/v1_2/V1_2_EXIT_GATE.md`).
- ☐ Exit gate (definition + protocol: `docs/research/v1_2/V1_2_EXIT_GATE.md`): cold-start Sol@high auditor in a fresh clone reproduces every calibration input from frozen snapshots; verdict filed in `docs/audits/`. Calibration itself excluded.
