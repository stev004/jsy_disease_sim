...
| **Astra tranche 2 — PERF-1: plain-metadata boundary outside Starsim discovery** | `perf/perf1-starsim-discovery-boundary` @ `7fe7df0`; merged via G18 | full-mode `Sim.init` 48.4 s → 1.07 s (≈47 s per replicate); 30-day run 73 s on a loaded box (Astra unloaded prototype 53.9 s; quiet-window remeasure owed) | ordered dist/rate paths, seeds, init arrays, latent/outcome/observation hashes, event order all equal ci+full × seeds 101/123 × 7/30 d — `docs/runs/2026-09-05-perf1-proof.json`; Sol review PASS |
| **Astra tranche 2 — PERF-2 (DATA-1): exact M3 secondary-job loop** | `perf/perf2-m3-secondary-jobs` @ `2a70915`; G18 | full-mode `generate_structure` 147 s → 5.1 s (fresh parent build only) | M3 hash `b7d2fb34…` unchanged; verbatim-original oracle incl. PCG64 state; seed 102 base==branch — `docs/runs/2026-09-05-perf2-m3-secondary-jobs-luna-report.md` |
| **Astra tranche 2 — PERF-3 (ROUTE-11 hash half): streamed canonical M4 hash** | `perf/perf3-framed-m4-hash` @ `7b64629`; G18 | hash-step transient allocation ~482 MiB → 3.98 MB; wall ≈ −0.4 s | 252,689,441 bytes byte-equal to the eager encoding; M4 hash `49464e77…` unchanged — `docs/runs/2026-09-05-perf3-framed-m4-hash-luna-report.md` |
| **ROUTE-5 phase 1: columnar `RouteSnapshot` + community builders emit columns** | `perf/route5-phase1-columnar-community` @ `6ff4870`; G18 | community routes 1.16–1.29× (SHA-256 per edge dominates); one full community_indoor snapshot 6.1 MB vs ~48 MB as dicts (LRU memory ≈8× smaller) | all 561 route/date fingerprints + `p1/p2/beta/dur` bytes identical on both windows; M4 hash; 7 d full + 30 d ci hashes — `docs/runs/2026-09-05-route5-phase1-columnar-luna-report.md`; Sol review 2 PASS |
| **ROUTE-4: workplace weekday memo** | `perf/route4-weekday-memo` @ `6ffceca`; G18 | workplace_team 6.38 s → 1.12 s per 30-date window (5.7×); transient 1.2× | 180-date equality vs the uncached reference; all fingerprints identical — `docs/runs/2026-09-06-route4-weekday-memo-luna-report.md` |

| **Ensemble-audit #3: pre-encoded community hash-key suffixes** | `perf/hashkey-suffix-preencode` @ `8c38e11`; merged G22 2026-09-08 | community routes 1.16–1.21× on the 30-date window | 10,000-triple payload equality; stable-int call counts equal base (13,779,583 / 9,494,324); fingerprints identical both windows; M4 + 7-day hashes unchanged — `docs/runs/2026-09-08-hashkey-suffix-preencode-luna-report.md` |
| **PERF-4: internal diagnostics mode for replicate M4 builds** | `perf/perf4-diagnostics-switch` @ `f60d165`; G22 | full-mode M4 build 29.05 s → 20.89 s median (ABAB ×5), −8.16 s per replicate | hashes identical across modes; default output byte-identical to base excl. timing fields; 2-seed pool ensemble == base — `docs/runs/2026-09-08-perf4-diagnostics-switch-luna-report.md` |
| **ROUTE-5 phase 2: transient / cross-class / bus builders emit columns** | `perf/route5-phase2-columnar-builders` @ `2dcc57b`; G22 | workplace_transient 1.35–1.40×, school_cross_class 1.37–1.49×, bus 1.96–1.99× | fingerprints identical both windows, stable-int counts equal; M4 + 7-day hashes unchanged — `docs/runs/2026-09-08-route5-phase2-columnar-builders-luna-report.md`; Sol review `docs/audits/2026-09-08-run4-perf-review-sol-PASS.md` |

| **PROV-8: one parent-build implementation** | `perf/prov8-one-parent-build` @ `284caff`; merged G23 2026-09-09 | none (refactor; −1 net line) | ci M2/M3/M4/ensemble/replicate/demo outputs identical — `docs/runs/2026-09-08-prov8-one-parent-build-luna-report.md` |
| **PERF-6: vectorised intervention route effects** | `perf/perf6-vectorised-interventions` @ `b720a49`; G23 | full 30-day `m7_combined` route-effect block 37.3 s → 11.4 s (3.28×); `m7_community_indoor` 15.7 s → 0.45 s (34.9×); baseline (no scenario) unaffected | 990-configuration schema grid + all 13 scenarios bit-identical arrays; full 30-day hashes + intervention parquet identical; review-caught targeted/residents_only defect fixed before merge — `docs/runs/2026-09-09-perf6-retry1-luna-report.md`, `docs/audits/2026-09-09-run4b-perf-review-sol-BLOCKED.md` → `…rereview-sol-PASS.md` |
| **PERF-9: verified parent reuse + pool initializer** | `perf/perf9-parent-reuse` @ `fba9fbf`; G23 | ensemble job payload 7,524,144 B → 43,200 B (initializer 1.78 MB once per worker); verified parent reuse across CLI/adapter invocations (ci: 4.87 s → 3.87 s) | sequential == pool == base replicate/ensemble hashes; resume authenticates — `docs/runs/2026-09-08-perf9-parent-reuse-luna-report.md`, `…perf9-retry1-luna-report.md` |
