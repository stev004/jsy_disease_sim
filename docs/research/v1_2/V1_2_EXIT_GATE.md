...
3. For every table in the enumerated set: pick ≥3 rows at random, follow `source_locator` back into the frozen file (CSV cell or PDF page) and confirm the value, the `reporting_status`, and the units; confirm the `source_sha256` equals `sha256sum` of the frozen file.
