...
## Erratum — 2026-09-20 (CROSS-3)

The horizon line and final latent headline dates above were misstated. The run
has **180 simulated dated points**, from **2025-01-06 through 2025-07-04**;
the observation summary is on a **184-date grid** extending through
**2025-07-08**. The frozen `ensemble_summary.parquet` confirms the four tail
dates have these cell semantics for the latent metrics:

| Latent metric | 2025-07-05 to 2025-07-08 |
|---|---|
| `latent_attack_rate` | `carried_forward` on all four dates |
| `latent_cumulative_incidence_per_capita` | `carried_forward` on all four dates |
| `latent_cumulative_infections` | `carried_forward` on all four dates |
| `latent_ever_infected_fraction` | `carried_forward` on all four dates |
| `latent_new_infections` | `structural_zero` on all four dates in this frozen pre-DISEASE-4 artifact |
| `latent_prevalence` | `outside_metric_horizon` on all four dates |

Ever-infected is constant across the four tail cells (`0.7778984120910657`),
so the final fraction and cumulative headline values were carried forward from
the last simulated date, **2025-07-04**, rather than generated by additional
simulation. The latent peak date, **2025-02-03 (point 29)**, was read from the
simulated series. The corrected horizon wording does not modify the frozen
artifact.
