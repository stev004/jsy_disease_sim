ROLE: Mechanical implementation, gpt-5.6-luna @ xhigh. G30 extension authorized by Steven: "alright keep iterating and working away" after the stop and gate digest. Attempt 4, one explicit extension; do not disguise as a new unit.
GOAL: Correct declared-decimal recovery arithmetic at the truth-joined layer, preserving the frozen scientific thresholds and all blind-fit behavior.
SCOPE: New isolated worktree /home/steven/jos-p01-numeric-fix-wt, branch codex/v13-p01-numeric-fix, exact base b5ef032e29c577bce634ce0933b2d7d316ac562e. Touch only src/jersey_outbreak/phase0_campaign.py and tests/test_phase0_campaign.py. AGENTS.md is your contract.
CONTEXT: Three prior implementations passed ordinary tests but missed an inclusive decimal endpoint. Sol's full numerical report below prescribes exact arithmetic for finite declared decimal grid values. No campaign has run. Earlier director acceptance is withdrawn. Existing configs, hashes, schemas, objective/profile arithmetic and blind boundary stay fixed. This is a numerical contract correction, not scientific retuning.
ACCEPTANCE:
1. Before changing production code, demonstrate behavioral test failures on base: upper asymptomatic endpoint .40 minus truth .25 is .15 inclusive; five identified rows with one upper endpoint have full marginal/joint coverage; a decisive 3/5 joint-hit construction passes; .40/.10 plus three truth estimates cancel exactly.
2. Use Decimal(str(value)) for direct declared grid estimates/truth/tolerance/bias limits; retain exact signed/absolute error, sum, mean and inclusive comparison arithmetic through recovery/bias evaluation. Emit floats only at existing output fields. Do not use an epsilon, arbitrary rounding or Decimal semantics on computed objectives.
3. Table-test both endpoints of all four frozen dimensions. Preserve failed/non-identified row handling and fixed denominators. Preserve blind estimate/config/declaration hashes and existing serialized types/contracts; add meaningful regression evidence for the unchanged blind boundary.
4. Existing focused and full tests and static gates pass on FINAL edits. Read changed implementation before reporting. No hidden source edits after full checks without rerunning affected checks.
5. Record the exact before/after commands and output, final diff stat, scope check, and any unresolved numerical limits. The profile-gap exact-boundary question from Sol is excluded: do not change objective/profile rules or invent its semantics.
VERIFY: export PATH="$HOME/.local/bin:$PATH"; uv run pytest tests/test_phase0_campaign.py; uv run pytest; uv run ruff check src/jersey_outbreak/phase0_campaign.py tests/test_phase0_campaign.py; uv run ruff format --check src/jersey_outbreak/phase0_campaign.py tests/test_phase0_campaign.py; uv run mypy src/jersey_outbreak/phase0_campaign.py; git diff --check. Tests may mock expensive execution, using actual config constructors and faithful result metadata; no real campaign. Director separately runs the CI verify mirror and independent Sol review before any feature push.
TIMEBOX: 25 minutes, one authorized extension attempt only. Stop with partial report on expiry or scope conflict.
FORBIDDEN: Edits outside named files; scientific retuning; new dependencies; YAML/spec/hash/schema changes; objective/profile changes; simulator/observation runs, pilots, real-data fitting or full-scale work; git commit/push; frozen evidence changes; implementation in primary checkout; filter bypass/retry.
REPORT: Per-criterion evidence and actual output, files changed/diff stat, explicit unresolved issues. Do not claim Phase-0 PASS. Director files final report and commits.
STANDING:
