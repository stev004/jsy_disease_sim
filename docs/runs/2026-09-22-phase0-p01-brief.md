TASK: JOS Phase 0 P0-1 recovery campaign implementation
ROLE: Implementer, gpt-5.6-luna xhigh. Risk: high (scientific acceptance machinery). You are not the independent judge.
GOAL: Implement P0-1 exactly from the frozen Sol predeclaration below: a standalone joint synthetic-recovery harness, blind fitting boundary, pure scoring/diagnostics, budget guard and research output. Do not run the campaign.
SCOPE: Work only in /home/steven/jos-p0-1-wt on codex/v13-p0-1-recovery, base 2ab6b10216b46e9599d9cc42c0b00aa9178c0436 (code baseline 3ff37348f88750470be9f1ccda193b3ff31fb9c3). Add only src/jersey_outbreak/phase0_campaign.py, configs/calibration/v13_phase0_synthetic.yaml, tests/test_phase0_campaign.py. No changes to any existing source, tests, schemas, CLI, dependencies, .claude/, AGENTS.md or docs. All scratch output outside tracked paths or under outputs/. No implementation of P0-2/P0-3 yet; their complete numerical specification is frozen now so later arms cannot adapt to P0-1 outcomes.
CONTEXT: The full written Sol specification follows. It was committed on main at a4b3200967f34e449bd7ffd4f589a8d30e841e95 before implementation, document SHA256 ef67fe49903c3984ca98679eb0470878bc25523baca3bc23a63e4ae7d983a104. Read-only exact copy at /home/steven/jos-p0-predeclaration.md; use that external path for --predeclaration because this unit's earlier base does not contain the new state document. Never cherry-pick a state commit. AGENTS.md is your constitution. Current calibration is scalar; preserve all existing APIs/contracts and reuse existing helpers where they support the specification. The baseline calibration/C3 tests passed (12 tests); this is not the Phase-0 gate. P0-1 must be implemented before the two subsequent units, then fresh Sol code review before one all-arms campaign, then a separate fresh Sol result audit.
ACCEPTANCE:
1. Encode every P0-1 numerical choice and predicate verbatim from the frozen spec, including the four-dimensional 81-cell exhaustive fit, independent target/candidate streams, pure observed-data-only fitter, complete calendar, tie handling, profile gap, descriptive coverage/bias and boundary accounting. No truth or target latent data may enter scoring/selection; write/hash estimates before joining truth evaluation.
2. A dry-run performs zero simulation/observation calls. It exposes P0-1 counts of 81 cells, 5 targets, 3 fitting seeds, 32 latent calls and 248 observation transforms, plus the frozen full-campaign caps. It rejects disallowed modes/horizons, overlapping seeds, oversized/undeclared grids, retries and replacements. Execute must fail closed while required arms are not implemented; never silently execute only P0-1 and label Phase 0 complete.
3. Tests use deterministic small fabricated numerical inputs and mocks, not a hidden campaign. Demonstrate arithmetic against hand-computed objective/coverage/bias examples, complete-grid enforcement, truth-blind interface, channel/date completeness, ties/near-flat profiles, boundary/exhaustion/error handling, chronology/conservation failure propagation and all PASS/FAIL predicates. For any reproduced existing bug, demonstrate failure on base before requesting an out-of-scope fix; do not fix protected modules yourself.
4. The output is a standalone research bundle with provenance and file-level hashes per spec, never a scalar CalibrationArtifactManifest. Preserve nulls/failed/non-identified states. All emitted summaries must derive from measured predicates. No real-data loading, nominal-coverage or calibration claims.
5. Scope stays exactly three new paths. Existing scientific behavior, all hashes/schema identities and default parameters are untouched. No new dependencies, speculative parallelism, second implementation of existing profiling logic, unrelated coverage or refactoring. An API conflict means stop and report rather than widening scope.
VERIFY:
export PATH="$HOME/.local/bin:$PATH"
export STARSIM_INSTALL_FONTS=0 MPLCONFIGDIR=/tmp/mpl-cache
uv run python -m jersey_outbreak.phase0_campaign dry-run --config configs/calibration/v13_phase0_synthetic.yaml --predeclaration /home/steven/jos-p0-predeclaration.md
uv run pytest tests/test_phase0_campaign.py -k 'p0_1 or blind or budget'
uv lock --check
...
uv run pytest
Run the unchanged .github/workflows/ci.yml verify-job commands locally, including the exact pinned mypy module list and relocation check, where feasible. No GitHub CI. Do not commit to satisfy clean-tree gates: record intended three uncommitted files; director will run the full mirror on a clean committed verification clone before push/review. The known test_prov_job_ordering::test_missing_head_request_fails_alone timing flake is not permission to suppress tests; report exact failure and let director confirm on base.
Persist command output to /home/steven/jos-p0-1-evidence.log (or /tmp/jos-p0-1-evidence.log if sandbox blocks home). git diff --check; git status --short; git diff --stat (include untracked file line counts). No campaign execute command, even to test end-to-end; use pure/mocked acceptance tests for new campaign behavior.
TIMEBOX: 75 minutes. One implementation attempt of maximum three. If impossible within this scope, stop with the exact contract conflict and partial evidence. Do not spawn subagents.
FORBIDDEN: Actual campaign/pilot fitting; any real Jersey fit; full-scale runs; changing numerical design after results; protected schema/hash/identity/lifecycle/provenance changes; existing module changes; new dependencies; git commit/push/merge/tag/reset/clean/branch deletion; credentials; frozen data/evidence writes; editing the predeclaration; calling the gate passed because unit tests pass.
REPORT: Verdict first IMPLEMENTATION READY FOR REVIEW or IMPLEMENTATION BLOCKED. Give per-criterion actual commands/results, artifact and transcript paths, diff stat/files, remaining limitations, explicit no-campaign-executed attestation. Do not claim independent or scientific PASS.
FROZEN SCIENTIFIC PREDECLARATION (verbatim):# READY FOR IMPLEMENTATION
...
STANDING (DIRECTOR.md verbatim; current Steven routing above overrides older seat defaults):
