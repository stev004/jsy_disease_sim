TASK: P0-1 numerical contract peer review (consult 2/2).
ROLE: Independent scientific/numerical reviewer, gpt-5.6-sol @ high, read-only. No implementation or subagents.
GOAL: Assess the reproduced inclusive-boundary defect and give the smallest correct remedy without changing the frozen scientific predicates.
SCOPE: Read-only clone /home/steven/jos-p01-numeric-readonly detached at b5ef032e29c577bce634ce0933b2d7d316ac562e. Inspect phase0_campaign.py, its tests/config and frozen specification. No file writes inside clone, no commits/pushes. Deterministic arithmetic probes outside clone are allowed; no pytest, simulations, observation transforms, fits or campaign/pilot execution.
CONTEXT: Three Luna P0-1 attempts spent. Final candidate has 22 focused tests and 432 full tests passing; director clean CI verify command mirror passed before this consult (filed in main 344d692f7ac36cfb349315232c405798c9420c6a, log text available at /home/steven/jsy_disease_sim/docs/runs/2026-09-22-phase0-p01-ci-mirror.log). Local uv is 0.12.9 vs CI setup 0.11.30; locked dependencies and command list checked, so do not claim identical tool environment. No candidate push. Director acceptance was held after a pure probe found candidate .40 minus truth .25 returns .15000000000000002, so RecoveryRow.absolute_error <= declared .15 is False. The mathematical declared endpoint is inclusive. The original specification is immutable and no real campaign has run. Root director may implement only an obvious one-line fix; anything larger needs a fourth Luna attempt authorized by Steven. Your task is diagnosis and a precise remedy recommendation, not to force a one-line solution or authorize an extra attempt.
ACCEPTANCE:
1. Confirm or refute the endpoint defect on this exact SHA and identify affected recovery/joint/bias comparisons and any other directly related numerical boundary issues.
2. Recommend the smallest robust representation/comparison approach that implements the existing declared numbers and inclusive predicates. Do not invent epsilon thresholds, loosen scientific tolerance, round away meaningful differences or retune anything. Explain limits for this frozen grid versus generic inputs.
3. State whether a genuinely obvious one-line director fix suffices. If not, give a bounded mechanical correction scope and meaningful failing-before/passing-after acceptance cases for Luna. Do not implement it.
4. Preserve the blind boundary, original hashes/schemas, seed schedules, objective/profile definitions, statuses and declaration hash. Distinguish a numerical implementation defect from scientific campaign failure; no scientific campaign has run.
5. Check the existing focused tests' coverage of the affected arithmetic; report actual file:line references and any unknowns. This is a bounded numerical peer consult, not the all-arms code audit or exit verdict.
VERIFY: GIT_OPTIONAL_LOCKS=0 git rev-parse HEAD and git status --porcelain at start/end. Read relevant source. Pure arithmetic probes only; report exact command and output if used. No new environment/dependency installation required.
TIMEBOX: 10 minutes. Stop with partial findings on expiry; do not expand scope. This consumes the second and final P0-1 peer consult under the current budget.
FORBIDDEN: Source/test/config edits, new numeric assumptions or thresholds, protected contract changes, actual campaign/pilot, real-data fitting, full-scale runs, commits/pushes, external messages, claiming a scientific exit verdict.
REPORT: NUMERICAL CONTRACT: PASS or NUMERICAL CONTRACT: BLOCKED first; then findings, smallest remedy, explicit whether >one line, bounded regression cases and clean-tree attestation. Final response only; director files it under docs/audits.
STANDING: Full DIRECTOR.md and frozen specification follow.
...
## Director probe
2026-09-22T02:13:22+01:00
b5ef032e29c577bce634ce0933b2d7d316ac562e
DECLARED_GRID_ENDPOINT 0.4
DECLARED_TRUTH 0.25
ABSOLUTE_ERROR 0.15000000000000002
DECLARED_TOLERANCE 0.15
COUNTED_WITHIN_TOLERANCE False
NO_CAMPAIGN_EXECUTED
