TASK: Phase-0 P0-2 pre-execution tie/missing-component clarification.
ROLE: Scientific consultant, gpt-5.6-sol @ high, read-only. No subagents. No implementation, tests, simulation, fits or pilot results.
GOAL: Give a bounded, written interpretation of P0-2 tied-minimum handling consistent with the frozen specification, before implementation or campaign results.
SCOPE: Read-only clone /home/steven/jos-p02-clarify-readonly, detached 29d44485c0871b57524a4a26ce38262e57f4ec04. No tracked or untracked writes inside the clone. Final report only through your final response; director files it under docs/audits/.
CONTEXT: P0-1's final correction is in flight; no real campaign arm has run. The frozen specification explicitly makes P0-1 global ties fail, while P0-2 distinguishes software_status from misspecification_detection and defines R_i, E_i and selected-candidate error predicates. It does not explicitly state what to report/count when a wrong-model minimum is tied and an estimate or E_i is undefined. The implementation must not invent a truth-informed tie break or silently count unknown diagnostics as detection. Your role is to interpret the existing contract, not redesign it. Specification full text follows, SHA256 ef67fe49903c3984ca98679eb0470878bc25523baca3bc23a63e4ae7d983a104.
ACCEPTANCE:
1. State whether the contract supports an unambiguous implementation or needs a pre-execution owner ruling. No new thresholds, seeds, grids, fitted dimensions or scientific criteria.
2. For each P0-2 arm, define software status versus scientific detection when all declared cells complete but minima tie, R_i exists, and selected-estimate or E_i components cannot be uniquely computed.
3. Explain how such targets enter the fixed five-target 3/5 and 4/5 aggregate predicates. Preserve explicit unknowns. If a choice would add a scientific rule rather than interpret one, flag it instead of silently adding it.
4. Give two or three deterministic edge-case examples using symbolic/abstract losses; no simulation/pilot. Do not infer numerical performance.
5. Cite the exact governing specification passages and attest no source/data changes or execution. Return a concise implementation-facing addendum, suitable to append unchanged to a brief.
VERIFY: git rev-parse HEAD; git status --porcelain at start and end with GIT_OPTIONAL_LOCKS=0. Read the specified declaration and standing orders only as needed. No test commands or campaign execution.
TIMEBOX: 10 minutes. Stop with partial findings or unresolved question on expiry. This is P0-2 scientific consult 1 of 2.
FORBIDDEN: Implementation; file writes; source/schema/hash changes; changing the frozen document; new scientific thresholds or defaults; fitting real data; campaigns/pilots; external messages; commits/pushes; retuning.
REPORT: READY CLARIFICATION or NEEDS OWNER RULING first. Then exact interpretation, limitations, deterministic examples and clean-tree attestation. This is not a code review or exit-gate verdict.
STANDING: Full DIRECTOR.md and frozen scientific specification follow.
