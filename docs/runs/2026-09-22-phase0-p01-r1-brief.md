TASK: JOS Phase 0 P0-1 recovery campaign implementation · retry 1
ROLE: Implementer, gpt-5.6-luna xhigh. Risk: high. This is attempt 2 of 3 for P0-1, not a new unit. No subagents.
GOAL: Complete and correct P0-1 against the unchanged frozen Sol predeclaration, closing all six director findings below, while keeping campaign execution blocked until P0-2/P0-3 and independent review.
SCOPE: /home/steven/jos-p0-1-wt, codex/v13-p0-1-recovery, exact unaccepted checkpoint 1fe0b970ba834bd0e5ae69d5ee244e609a41445a. Modify only src/jersey_outbreak/phase0_campaign.py, configs/calibration/v13_phase0_synthetic.yaml, tests/test_phase0_campaign.py. All existing source/schema/artifact/CLI modules, dependencies, .claude/, AGENTS.md, docs and frozen data/evidence remain read-only. The checkpoint was made by the director solely for immutable before/after proof; it was not kept or pushed. The branch now contains the committed predeclaration at docs/research/v1_3/2026-09-21-phase0-predeclaration.md.
CONTEXT: First attempt implemented useful pure scoring but did not implement the simulator adapter needed by the campaign. The director owns the brief ambiguity: DO NOT RUN THE CAMPAIGN does not mean DO NOT IMPLEMENT its generation/orchestration path. Implement that callable path now with mocked integration tests; leave the public all-arms execution guard blocked. The original scientific declaration is unchanged, SHA256 ef67fe49903c3984ca98679eb0470878bc25523baca3bc23a63e4ae7d983a104, frozen in a4b3200967f34e449bd7ffd4f589a8d30e841e95 before results. No campaigns have run. The first report claimed 13 focused + 422 full tests; some final changes followed the full suite. Those tests did not prove scientific acceptance. All confirmed findings and exact probe output follow this brief.
ACCEPTANCE:
1. Failing-first: before editing implementation, add bounded regression tests for each confirmed executable defect. Run them on checkpoint code and retain actual failures, then show the same tests pass after fixes. Initial date/horizon defects were already closed by your last pass; preserve those fixes. Do not weaken existing tests/assertions. At minimum: modified profile gap/coverage declaration rejected; missing workload/undeclared seeds cannot PASS; repeated bundle output cannot corrupt existing bytes; portable predeclaration lookup and estimate-persistence ordering.
2. Implement the P0-1 target/candidate generation adapter and callable orchestration within the existing module, using existing real JOS APIs and the exact fixed configs/seeds from the specification. Produce observed two-channel tables, reuse the predeclared candidate latent calls, keep target and prediction streams disjoint, score all cells through fit_blind, persist/hash blind estimates before joining truth, derive chronology/conservation/namespace checks from actual outputs/configs, and return the data needed for later arms. Test this entire path with mocks/fakes at the existing API boundaries, including call order, seeds, complete calendars, all declared cell/replicate counts, and failure handling. Do not introduce a speculative backend framework or duplicate existing simulation logic. Do not invoke the real generation path or run any campaign/pilot. CLI execute stays fail-closed while required arms are absent.
3. Validate every scientifically relevant frozen YAML field, including currently ignored objective/identifiability/acceptance/observation/seed-option/budget fields; reject contradictory, undeclared or malformed declarations plainly. No silent coercion that changes the declaration. Candidate provenance must hash the actual complete constructed configs, including the relevant seed namespaces and fixed parameters, rather than a manually hard-coded subset. A meaningful config change must change its provenance or be rejected; irrelevant output paths must not masquerade as scientific changes.
4. Required evidence is mandatory. A missing measured workload is unknown/failure, never PASS. Verify target seeds against the declared set, actual cell/replicate completeness, matching declared calendars/configs and all required diagnostics. Defaults of True must not manufacture namespace/completion evidence. The blind fit boundary remains observed data + candidates only; no target truth or latent metadata enters scoring. A failed blind-estimate write prevents truth join/evaluation. A valid mocked execution must still be capable of reaching PASS, so this is not solved by making every result fail.
5. Tests must run in a clean checkout on another machine without /home/steven files. Use the committed repository-relative predeclaration or temporary fixtures. Its real hash remains verified by the CLI dry-run. Objective fixtures should include distinct fitting-replicate counts so averaging square roots is tested rather than accidentally matching square root of the average. Keep tests bounded and purposeful.
6. Research bundle creation rejects a nonempty destination before changing any bytes; no new resume/overwrite mode. Verify every SHA256SUMS entry on first write; do not hash the checksum file into itself. Missing evidence cannot produce a scientific PASS or a calibration claim. Keep nulls/failures explicit. Preserve existing scalar artifact contracts, hashes and schemas.
7. Scope remains exactly the same three paths. No P0-2/P0-3 implementation yet; no scientific parameter/tolerance/seed changes; no new dependencies, optimizations or unrelated refactors. No real-data access or fitting. Honor AGENTS plain CLI error paths and exit-code/message checks. If real APIs cannot support the fixed spec without a protected-contract change, stop and report the exact conflict.
VERIFY:
export PATH="$HOME/.local/bin:$PATH"
export STARSIM_INSTALL_FONTS=0 MPLCONFIGDIR=/tmp/mpl-cache
First capture new regression failures against untouched checkpoint implementation; after fixes run the same tests, then:
...
uv run pytest
Run the exact pinned mypy module list and relocation check from .github/workflows/ci.yml. Do not use GitHub CI. The director will run the full mirror from a clean committed clone before push/review; report intended dirty files instead of committing to make a clean-tree check pass. Existing known job-ordering timing flake must be reported verbatim, not suppressed; director confirms it on base if encountered. Save all before/after and gate output to /tmp/jos-p0-1-r1-evidence.log. git diff --check; git diff --stat; git status --short. No real campaign execute, including hidden execution in tests.
TIMEBOX: 75 minutes. Stop with partial findings on expiry or a scope conflict. This attempt spends implementation run 2 of 3; no scientific consult or model escalation is authorized by this brief.
FORBIDDEN: Real campaign or pilot execution; real-data fitting; changing the predeclaration; numerical retuning; protected-contract or existing-module changes; new dependencies; edits outside the three paths; git commit/push/merge/tag/rebase/reset/clean/branch deletion; frozen evidence/data writes; credentials; declaring the scientific gate passed because tests pass.
REPORT: IMPLEMENTATION READY FOR REVIEW or IMPLEMENTATION BLOCKED first. Give per-finding failure-before/success-after commands and actual output, final-source gate results, files/diff stat, transcript path and remaining limitations. Explicitly attest no campaign run and no protected modules changed. If full-suite evidence predates later edits, say so accurately.
DIRECTOR REVIEW (verbatim):# P0-1 director acceptance review — FAIL
...
FROZEN SOL SPECIFICATION (verbatim):
