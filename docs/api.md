# Milestone 9 local API and jobs
...
`SUCCEEDED` only after its scientific artifact verifier passes and the M9
result manifest has been finalized. Failed or partial output is not exposed as
a verified successful result.
...
reruns them. On startup, a valid result manifest whose database transition was
interrupted is reconciled only after its job-owned scientific artifacts are
verified again. Otherwise stale active jobs become `INTERRUPTED`.
...
`running`, `writing_artifacts`, `verifying`, and `complete`. Fractional
...
scientific artifact manifest. It records the job ID/kind, API request hash,
worker-observed engine commit, dirty-worktree flag, artifact roles, scientific
...
intervention, scope, metric, key, and seed. Responses default to 1,000 rows
and have a hard maximum of 10,000 rows. Ordering is deterministic by date and
canonical row content. JSON serialization converts non-finite numeric values
to `null`.
...
work. The direct `jos` scientific CLI remains independent and supported.
