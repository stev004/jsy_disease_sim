...
def test_explicit_horizon_tail_allows_right_censoring(m6_latent_run, m6_observation_config) -> None:
    too_short = m6_observation_config.model_copy(
        update={
            "detection_delay": m6_observation_config.detection_delay.model_copy(
                update={"days": (1,)}
            ),
            "reporting_delay": m6_observation_config.reporting_delay.model_copy(
                update={"days": (0,)}
            ),
            "analysis_horizon_tail_days": 0,
        }
    )
    result = observe_latent_run(m6_latent_run, too_short)
    assert result.diagnostics["analysis_horizon"]["tail_days"] == 0
...
def test_observation_parish_and_age_aggregates_reconcile(
    ...
    assert result.diagnostics["ascertainment_fraction"] == pytest.approx(
        result.diagnostics["detected_event_count"] / len(result.observation_events)
    )
...
def test_observation_artifact_contains_provenance_and_tidy_tables(
    ...
    cases_schema = pq.read_schema(artifact.artifact_directory / "daily_observed_cases.parquet")
    assert "ascertainment_fraction" not in cases_schema.names
    assert {
        "cohort_detected_infections",
        "cohort_ascertainment_fraction",
        "cohort_ascertainment_censored",
        "cohort_detection_window_end_date",
    } <= set(cases_schema.names)
    for filename in (
